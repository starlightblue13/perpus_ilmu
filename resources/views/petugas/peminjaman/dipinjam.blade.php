...
<h2 class="text-2xl font-bold mb-5">Tab : Dipinjam</h2>
    ...
    <table class="w-full border border-gray-300 text-left">
        ...
        <tbody>
            ...
            <tr class="{{ $loop->odd ? 'bg-[#E2EDF9]' : 'bg-[#C7D8E8]' }}">
                ...
                <td class="p-3 border">{{ ucfirst($p->status) }}</td>

                <td class="p-3 border">

                   
                        {{-- Setelah klik ingatkan --}}
                    @if($p->status === 'Dipinjam')
                             <form action="{{ route('petugas.peminjaman.terima', $p->id) }}" 
                    ...
                    @else
                        {{-- Normal --}}
                        <a href="?ingatkan={{ $p->id }}"
                           class="px-4 py-1 bg-orange-500 text-white rounded">
                            Ingatkan Kembalikan
                        </a>
                    @endif
...
<div class="fixed inset-0 bg-black bg-opacity-50 backdrop-blur-sm flex items-center justify-center z-[9999]">
    ...
    <div class="bg-white w-[430px] p-8 rounded-2xl shadow-2xl text-center">
        ...
        <h2 class="text-2xl font-bold mb-3">Buku Harus Dikembalikan?</h2>
        <p class="text-gray-600 mb-6">
            Peminjam akan diberi pemberitahuan agar buku segera dikembalikan.
        </p>

        <form action="{{ route('petugas.peminjaman.ingatkan', $ingatkanId) }}" method="POST">
            @csrf
